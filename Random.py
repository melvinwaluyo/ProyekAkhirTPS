...
class Person:
    def __init__(self, x, y, elevators, entry_point):
        ...
        self.target_elevator = random.choice(elevators)
        self.waiting = False
    ...
    def update_target_elevator(self):
        ...
        if available_elevators:
            self.target_elevator = random.choice(available_elevators)
            self.waiting = False
        else:
            self.waiting = True
            self.stop_x = self.x
    ...
    def move(self):
        if self.state == 'moving_vertical':
            ...
            # Move towards the center of the target elevator
            target_x = self.target_elevator.x + 25
            target_y = self.target_elevator.y + 25
